from src.account.utils import generate_success_code, generate_current_date
...
def retrieve_user_password(user_particulars: Particulars) -> dict:
    """
    Attempts to retrieve a user's password. This is the first out of two APIs that should be called in order to retrieve
    the password of a user. An OTP will be sent to the user's mobile number for authentication, and the second API
    ...
    """
    logger.info("Attempting to retrieve user password.")
...
def authenticate_password_retrieval(user_particulars: Particulars) -> dict:
    """
    Authenticates the retrieval of a user's password. This is the second out of two APIs that should be called in order
    to retrieve the password of an existing user. The contents of the input must include the user's birth date and
    mobile number, as well as the success code from the first API. The transaction id and the OTP entered by the user
    must also be included. If successful, the user's account will be created. This method should not be called directly,
    but rather only with the API call provided in mainapp.
    ...
    :return: Returns the user's current plaintext username, encrypted username, and encrypted password.
    """
    logger.info("Attempting account authentication for user password retrieval.")
    ...
    password = user_from_database.get_parameter_value(PARAMETER_NAME_PLAINTEXT_PASSWORD)
    ...
    logger.info("Authentication of password retrieval completed.")
